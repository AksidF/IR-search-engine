...
class TFIDF:
    ...
    def __init__(self, **kwargs):
        self.word_counter = kwargs.get("word_counter", None)
        self.index_to_word = kwargs.get("index_to_word", None)
        self.word_to_index = kwargs.get("word_to_index", None)
        self.document_frequency = kwargs.get("document_frequency", None)
        self.n_docs = kwargs.get("n_docs", 0)
    ...
    def _clean_data(self, data: list[str]) -> list[str]:
        ...
        return [
            " ".join(re.findall(r"[a-zA-Z0-9]+", sentence)).lower() for sentence in data
        ]
